...
assert sys.version_info.major == 3 and sys.version_info.minor >= 5, "Python version must be >= 3.5"
...
def main(args: argparse.Namespace):
    ...
    installers += [
        Shell(
            "installer/install-zsh.sh",
            install_check=lambda: (util.shell_command("which zsh") and util.check_zsh_version()),
        )
    ]
    if util.is_osx():  # OSX packages
        installers += [
            Shell(
                "installer/install-brew.sh", install_check=lambda: util.shell_command("which brew")
            ),
        ...
        installers += [
            ...
            Shell(
                "installer/install-ripgrep.sh", install_check=lambda: util.shell_command("which rg")
            ),
        ]
    # tmux
    installers += [
        Shell(
            "installer/install-tmux.sh",
            install_check=lambda: (
                util.shell_command("which tmux") and util.shell_output("tmux -V") == "tmux 2.6"
            ),
        )
    ]
    # pip
    installers += [
        PipPackage("neovim"),
        PipPackage("pylint"),
        PipPackage("python-language-server"),
    ...
    installers += [
        Shell(
            "installer/install-oh-my-zsh.sh",
            install_check=lambda: os.path.exists(os.path.join(util.home_dir(), ".oh-my-zsh")),
        )
    ]
    # symlinks
    installers += [
        Symlink(sym_path, args.backup) for sym_path in glob.glob("**/*.symlink", recursive=True)
    ]
    # sourced symlinks
    installers += [SourceSymlink("zsh/zshrc.global.symlink")]
    # link nvim configs
    nvim_config_dir = os.path.join(util.home_dir(), ".config", "nvim")
    nvim_link_dir = os.path.abspath("vim/vim.symlink")
    installers += [
        Shell(
            "installer/install-nvim-links.sh",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--uninstall", action="store_true", help="Uninstall dotfiles.")
